...
// Config is loaded entirely from the environment so a systemd unit can drive it
// via an EnvironmentFile. No config file, no flags.
type Config struct {
	...
	FromDomain  string        // FROM_DOMAIN, optional; if set, off-domain From is rewritten
...
func loadConfig() (Config, error) {
	c := Config{
		...
		FromDomain:  os.Getenv("FROM_DOMAIN"),
		HTTPTimeout: time.Duration(envInt("HTTP_TIMEOUT_SECONDS", 30)) * time.Second,
		MaxSize:     int64(envInt("MAX_MESSAGE_BYTES", 25*1024*1024)),
	}
	if c.WorkerURL == "" {
		return c, fmt.Errorf("EMAIL_WORKER_URL is required (e.g. https://cf-email-relay.<acct>.workers.dev/send)")
	}
	if c.Token == "" {
