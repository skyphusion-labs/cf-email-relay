...
// the public HTTP endpoint (e.g. the SMTP relay). Validation lives here so both
...
export interface EmailRequest {
  to: string | string[];
  /** Defaults to env.DEFAULT_FROM. Must be on ALLOWED_FROM_DOMAIN if that is set. */
...
// Deliberately permissive; the upstream service does the real validation. We
// only reject obviously malformed addresses to fail fast with a clear code.
...
function resolveFrom(env: Env, from: EmailRequest["from"]): EmailAddress {
  // Optional: when ALLOWED_FROM_DOMAIN is empty/unset, any onboarded sender
  // domain is allowed (Cloudflare's domain onboarding is the real gate).
  const allowedDomain = (env.ALLOWED_FROM_DOMAIN ?? "").trim().toLowerCase();
  ...
  if (from === undefined) {
    if (!env.DEFAULT_FROM) {
      throw new EmailError(
        "E_FIELD_MISSING",
        "no from address provided and DEFAULT_FROM is not configured",
      );
    }
...
export async function sendEmail(env: Env, req: EmailRequest): Promise<SendResult> {
  ...
  // It throws an Error with a `.code` (E_* string) on failure; callers map it.
