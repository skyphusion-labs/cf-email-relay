...
// Failure codes that represent a transient upstream condition rather than a bad
// request; the public endpoint maps these to 502 so callers can retry.
...
/**
 * RPC surface for same-account Workers bound via a service binding. No network
 * hop, no shared secret.
 *
 *   // consumer wrangler.jsonc
 *   "services": [
 *     { "binding": "EMAIL", "service": "cf-email-relay", "entrypoint": "EmailService" }
 *   ]
 *
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && (url.pathname === "/" || url.pathname === "/health")) {
      return json({ ok: true, service: "cf-email-relay" });
    }
    ...
    // Bearer-token gate for the public endpoint (used by the SMTP relay and any
    // external caller that can't use a service binding).
